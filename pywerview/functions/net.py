...
class NetRequester(LDAPRPCRequester):
    ...
    def get_adobject(
        ...
        for attr_desc, attr_value in (
            ("objectSid", queried_sid),
            ("name", escape_filter_chars(queried_name)),
        ...
        for objectowner in objectowners_raw:
            ...
            if resolve_sids:
                try:
                    ...
                    try:
                        resolved_sid = self.get_adobject(
                            queried_sid=sid, queried_domain=self._queried_domain, attributes=["distinguishedname"]
                        )[0].distinguishedname
                    except IndexError:
    ...
    def get_netgmsa(
        ...
        if not self._ldap_connection.server.ssl:
            self._logger.warning(
                "LDAP connection is not encrypted, we can't ask "
    ...
    def get_objectacl(
        ...
        if resolve_guids:
            ...
            for o in self.get_adobject(
                ads_path="CN=Schema,CN=Configuration,{}".format(base_dn),
                attributes=["name", "schemaIDGUID"],
                custom_filter="(schemaIDGUID=*)",
            ):
                guid_map["{}".format(format_uuid_le(o.schemaidguid))] = o.name

            for o in self.get_adobject(
                ads_path="CN=Extended-Rights,CN=Configuration,{}".format(base_dn),
        ...
        security_descriptors = self._ldap_search(
            object_filter, adobj.ADObject, attributes=attributes, controls=controls
        )
        ...
        for security_descriptor in security_descriptors:
            ...
            for ace in sd[acl_type]["Data"]:
                if guid_filter:
                    try:
                        object_type = (
                            format_uuid_le(ace["Ace"]["ObjectType"])
                            if ace["Ace"]["ObjectType"]
                ...
                if resolve_sids:
                    ...
                    try:
                        ...
                        try:
                            resolved_sid = self.get_adobject(
                                queried_sid=converted_sid,
                                queried_domain=self._queried_domain,
                                ads_path=self._base_dn,
                ...
                try:
                    ...
                    pass
                try:
                    attributes["inheritedobjectacetype"] = (
                        format_uuid_le(ace["Ace"]["InheritedObjectType"])
                        if ace["Ace"]["InheritedObjectType"]
                        else "{00000000-0000-0000-0000-000000000000}"
                    )
    ...
    def get_netgroup(
        ...
        if not "*" in queried_groupname:
            ...
            self._logger.warning(
                '"*" detected in "{}", if it also contains "(",")" or "\\", '
        ...
        if queried_username:
            ...
            while sam_account_name_to_resolve:
                ...
                if first_run:
                    ...
                    objects = self.get_adobject(
                        queried_sam_account_name=sam_account_name, queried_domain=queried_domain
                    )
                    objects += self.get_adobject(queried_name=sam_account_name, queried_domain=queried_domain)
    ...
    def get_netcomputer(
        ...
        for attr_desc, attr_value in (
            ("servicePrincipalName", queried_spn),
            ("operatingSystem", queried_os),
            ("operatingsystemservicepack", queried_sp),
    ...
    def get_netdomaincontroller(self, queried_domain=str()):
        ...
        return self.get_netcomputer(
            queried_domain=queried_domain, full_data=True, custom_filter=domain_controller_filter
        )
    ...
    def get_netfileserver(self, queried_domain=str(), target_users=list()):
        ...
        if target_users:
            ...
            for target_user in target_users:
                users += self.get_netuser(target_user, queried_domain, attributes=file_server_attributes)
        else:
            users = self.get_netuser(queried_domain=queried_domain, attributes=file_server_attributes)
    ...
    def get_dfsshare(self, version=["v1", "v2"], queried_domain=str(), ads_path=str()):
        ...
        def _get_dfssharev1():
            ...
            intermediate_results = self._ldap_search(
                dfs_search_filter, adobj.ADObject, attributes=["remoteservername", "name"]
            )
            results = list()
            for dfs in intermediate_results:
                for remote_server in dfs.remoteservername:
                    ...
                    if "\\" in remote_server:
                        attributes = {"name": dfs.name, "remoteservername": remote_server.split("\\")[2]}
        ...
        def _get_dfssharev2():
            ...
            intermediate_results = self._ldap_search(
                dfs_search_filter, adobj.ADObject, attributes=["msdfs-linkpathv2", "msDFS-TargetListv2"]
            )
            results = list()
            for dfs in intermediate_results:
                attributes = list()
                ...
                for target in soup_target_list.targets.contents:
                    if "\\" in target.string:
                        ...
                        attributes = {"name": "{}{}".format(dfs_root, share_name), "remoteservername": server_name}
    ...
    @LDAPRPCRequester._ldap_connection_init
    def get_netou(self, queried_domain=str(), queried_ouname="*", queried_guid=str(), ads_path=str(), full_data=False):
    ...
    @LDAPRPCRequester._ldap_connection_init
    def get_netsubnet(
        self, queried_domain=str(), queried_sitename=str(), ads_path=str(), ads_prefix=str(), full_data=False
    ):
    ...
    def get_netgroupmember(
        ...
        def _get_members(_groupname=str(), _sid=str()):
            try:
                ...
                if _groupname:
                    self._logger.debug("Queried groupname = {}".format(queried_groupname))
                    groups = self.get_netgroup(
                        queried_groupname=_groupname, queried_domain=self._queried_domain, full_data=True
                    )
                ...
                # `--groupname` option is missing, falling back to the "Domain Admins"
                    ...
                    if _sid:
                        ...
                        domain_controller_filter = "(userAccountControl:1.2.840.113556.1.4.803:=8192)"
                        domain_controllers = self.get_netcomputer(
                            queried_domain=queried_domain,
                    ...
                    groups = self.get_netgroup(
                        queried_sid=queried_sid, queried_domain=self._queried_domain, full_data=True
                    )
            except IndexError:
            ...
            for group in groups:
                ...
                if recurse and use_matching_rule:
                    group_memberof_filter = (
                        "(&(samAccountType=805306368)(memberof:1.2.840.113556.1.4.1941:={}){})".format(
                            group.distinguishedname, custom_filter
                        )
                    )

                    members = self.get_netuser(custom_filter=group_memberof_filter, queried_domain=self._queried_domain)
                else:
                    # TODO: range cycling
                    try:
                ...
                for member in members:
                    ...
                    if is_group:
                        ...
                        self._logger.debug(
                            "{} is a group, ignoring the useraccountcontrol".format(member.samaccountname)
                        )
                    else:
    ...
    @LDAPRPCRequester._ldap_connection_init
    @LDAPRPCRequester._rpc_connection_init(r"\samr")
    def get_netlocalgroup(self, queried_groupname=str(), list_groups=False, recurse=False):
            ...
            for domain_sid, domain_handle in domain_handles.items():
                ...
                while True:
                    resp = samr.hSamrEnumerateAliasesInDomain(
                        self._rpc_connection, domain_handle, enumerationContext=enumeration_context
                    )
        ...
        # If we query a group
            ...
            try:
                group = samr.hSamrOpenAlias(
                    self._rpc_connection, queried_group_domain_handle, aliasId=queried_group_rid
                )
            ...
            for member in resp["Members"]["Sids"]:
                ...
                if (
                    ...
                    and self._domain_controller
                    and recurse
                    and attributes["isdomain"]
                    and attributes["isgroup"]
                ):
                    for domain_member in self.get_netgroupmember(
                        full_data=True, recurse=True, queried_sid=attributes["sid"]
                    ):
    ...
    def get_netprocess(self):
        wmi_enum_process = self._wmi_connection.ExecQuery("SELECT * from Win32_Process", lFlags=WBEM_FLAG_FORWARD_ONLY)
        while True:
            try:
                ...
                if wmi_process_owner != None:
                    attributes = {
                        "computername": self._target_computer,
    ...
    def get_userevent(self, event_type=["logon", "tgt"], date_start=5):
        ...
        wmi_enum_event = self._wmi_connection.ExecQuery(
            "SELECT * from Win32_NTLogEvent where {}" "and TimeGenerated >= '{}'".format(where_clause, limit_date),
        ...
        while True:
            try:
                ...
                if wmi_event_type == 4624:
                    ...
                    if logon_type not in [2, 3] or user.endswith("$") or (user.lower == "anonymous logon"):
                        continue
                else:
                    logon_type = str()
